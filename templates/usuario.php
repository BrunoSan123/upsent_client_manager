...
<body>
    ...
    <div class="filters">
        <form action="" method="post">
            <div class="filter-div">
                ...
                <input type="text" name="numero_chamado" id="numaro_chamado" class="input-paddings-1">
    ...
    <section>
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['numero_chamado'])) {
            $results = null;
            $total_tasks = null;
            $total_pages = null;
            $num_chamado = $_POST['numero_chamado'];
            $results = $wpdb->get_results("SELECT * FROM $table_name WHERE task_name='$num_chamado' ORDER BY id DESC LIMIT $posicao_inicial, $itens_por_pagina");
            $total_tasks = $wpdb->get_var("SELECT COUNT(*) FROM $table_name WHERE task_name='$num_chamado'");
            $total_pages = ceil($total_tasks / $itens_por_pagina);
        }
        ?>
        ...
        <?php foreach ($results as $result) : ?>
            ...
            <div class="upsent_table-mobile table-mobile">
                <div class="table_mobile_main">
                    ...
                    <div class="upsent-table-item"><span>Status:</span>
                        <div class="<?php if ($result->states == "parado") : ?>conclued_bullet<?php elseif ($result->states == "em_andamento") : ?>bullet-yellow<?php else : ?> bullet-green <?php endif ?>"></div>
                    </div>
                    <div class="upsent-table-item">Finalizar chamado:<div class="finished"></div>
                    </div>
...
$filterParams = "&num_chamado=" . $num_chamado;

if ($pagina_atual > 1) {
    echo "<a href='?page=usuario&pagina=" . ($pagina_atual - 1) . $filterParams . "'>Anterior</a>";
}
...
for ($j = 1; $j <= $total_pages; $j++) {
    if ($j == $pagina_atual) {
        ...
        echo "<a href='?page=usuario&pagina=$j" . $filterParams . "'>$j</a>";
    }
}

if ($pagina_atual < $total_pages) {
    echo "<a href='?page=usuario&pagina=" . ($pagina_atual + 1) . $filterParams . "'>Próximo</a>";
}
        ...
        <div class="upsent-pop-up" id="upsent-<?php echo $i ?>">
            <section>
                ...
                if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['submit-' . $i])) {
                    ...
                    list($horas, $minutos) = explode('.', $total_horas);
                    ...
                    $wpdb->update(
                        $report_table,
                        array(
                            'month_total_time' => $total_minutos,
