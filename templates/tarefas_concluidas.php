...
<body>
    ...
    <section>
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['user_filter'])) {
            ...
            $total_tasks = null;
            $total_pages = null;
            $user_param = $_POST['company_filter'];
            $results = $wpdb->get_results("SELECT * FROM $table_name WHERE funcionaro_responsavel='$user_param' AND entregue=1 ORDER BY id DESC LIMIT $posicao_inicial, $itens_por_pagina");
            $total_tasks = $wpdb->get_var("SELECT COUNT(*) FROM $table_name WHERE funcionaro_responsavel='$user_param' AND entregue=1");
            $total_pages = ceil($total_tasks / $itens_por_pagina);
        }
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['company_filter'])) {
            ...
            $results = $wpdb->get_results("SELECT * FROM $table_name WHERE company='$company_param' AND entregue=1 ORDER BY id DESC LIMIT LIMIT $posicao_inicial, $itens_por_pagina");
        ...
        <?php foreach ($results as $result) : ?>
            <table class="upsent_table table-desk">
                ...
                <tr class="upsent_table_data">
                    ...
                    <td>
                        <div class="<?php if ($result->concluida == 0) : ?> conclued_bullet <?php else : ?> bullet-green <?php endif ?>"></div>
                    </td>
                    <?php if ($result->entregue != 0) : ?>
                        <td>
            ...
            <div class="upsent_table-mobile table-mobile">
                <div class="table_mobile_main">
                    ...
                    <?php if ($result->concluida != 0) : ?>
                        <div class="upsent-table-item">Reabrir:<div class="finished"></div>
                        </div>
...
$filterParams = "&company=" . $company_param."&employer=".$user_param;
